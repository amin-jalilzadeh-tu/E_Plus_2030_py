...
#from Elec.elec_lookup import elec_lookup as default_elec_lookup
from Elec.lighting_lookup import lighting_lookup
from Elec.lighting_overrides_from_excel import (
    read_lighting_overrides_from_excel, 
    apply_lighting_overrides_to_lookup)
# DHW
from DHW.water_heater import add_dhw_to_idf
from DHW.dhw_lookup import dhw_lookup as default_dhw_lookup
from DHW import dhw_lookup as dhw_lookup_module
...
from HVAC.hvac_overrides_from_excel import (
...
from ventilation.ventilation_overrides_from_excel import (
...
from shading.shading import add_shading_to_idf  # <-- The new shading integration
...
def main():
    ...
    df_geom.to_csv("assigned_geometry.csv", index=False)
    ...
    df_lights.to_csv("assigned_lighting.csv", index=False)
    print(df_lights)
    ...
    df_dhw.to_csv("assigned_dhw_params.csv", index=False)
    ...
    for bldg_id, param_dict in assigned_fenez_log.items():
        # param_dict is the dictionary of {param_name => value or sub-dict}
        for p_name, p_val in param_dict.items():
            if isinstance(p_val, dict):
                # It's likely one of those sub-dicts (like "fenez_top_opq"), so flatten further
                # e.g. p_val = {"Name": "Concrete_200mm", "Thickness": 0.2, ...}
                for sub_k, sub_v in p_val.items():
                    combined_key = f"{p_name}.{sub_k}"  
                    fenez_rows.append({
                        "ogc_fid": bldg_id,
                        "param_name": combined_key,
                        "assigned_value": sub_v
                    })
            else:
                # It's a direct scalar or tuple (like "fenez_wwr": 0.3 or "fenez_wwr_range": (0.3,0.35))
                fenez_rows.append({
                    "ogc_fid": bldg_id,
                    "param_name": p_name,
    ...
    csv_path = "assigned_fenez_params.csv"
    ...
    df_hvac.to_csv("assigned_hvac_params.csv", index=False)
    ...
    df_vent.to_csv("assigned_ventilation.csv", index=False)
    ...
    df_epw_assigned.to_csv("assigned_epw_paths.csv", index=False)
    ...
    df_ground.to_csv("assigned_ground_temps.csv", index=False)
    print(df_ground)
    ...
    df_setzone.to_csv("assigned_setzone_params.csv", index=False)
    print(df_setzone)
